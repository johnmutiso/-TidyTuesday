...
cost_potential_pay_1 <- cost_potential_pay %>% 
  mutate(early_career_cat = factor(case_when(early_career_pay < 50000 ~ 'Below \n$50,000',
         ...
         state = factor(state),
         type = factor(type)) %>% 
  select(state, type, degree_length, early_career_cat, mid_career_cat, in_state_total_cat, out_state_total_cat)
...
plot <- cost_potential_pay_1 %>% 
  ...
  labs(y = "Number of Institutions", 
       title = paste0("SALARY($) POTENTIAL BASED ON TUITION FEES [",toupper(tuition_state_cat), "-STATE]"), 
       subtitle = paste0("[",toupper(tuition_state_cat), "-STATE]", "Residents"), 
